...
3. **완료 전 (생략 금지)**: `rtk gradlew clean build --no-daemon`(Spotless 포함), `rtk gradlew cleanTest test --no-daemon` 실행. Gradle 캐시 결과만으로 통과를 판단하지 않는다. 실패 시 수정 후 재검증한다. 환경 문제 또는 RTK 출력 문제로 실행 결과를 확인할 수 없으면 원문 Gradle 명령으로 재실행하고 사유를 보고한다.
...
- `rtk`를 사용할 수 없거나, 출력 세션이 끊기거나, 종료 코드·테스트 리포트를 확인할 수 없으면 원문 Gradle 명령으로 즉시 재실행하고 그 사실을 결과에 기록한다.
